...
M <- function(data, group, unit, by = NULL, contribution.from = NULL, cores = NULL) {
  ...
  if (!is.null(by))  {
    ...
    if (!is.null(cores) && tolower(Sys.info()["sysname"]) != "windows" && cores > 6) {
    ...
    if (!is.null(contribution)) {
      ...
      index_total <- data.table(M = M_value(data = data_tmp, group =  group, unit = unit))

      if (!is.null(cores) && tolower(Sys.info()["sysname"]) != "windows" && cores > 6) {
        M_contribution <- mclapply(contribution, function(c) {
